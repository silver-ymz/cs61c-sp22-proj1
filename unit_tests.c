...
bool test_is_tail() {
  ...
  if (!assert_true("e is not tail", !is_tail('e'))) {
    return false;
  }
...
bool test_is_snake() {
  if (!assert_true("w is snake", is_snake('w'))) {
    return false;
  }

  if (!assert_true("a is snake", is_snake('a'))) {
    return false;
  }

  if (!assert_true("< is snake", is_snake('<'))) {
    return false;
  }

  if (!assert_true("x is snake", is_snake('x'))) {
    return false;
  }

  if (!assert_true("e is not snake", !is_snake('e'))) {
    return false;
  }
...
bool test_body_to_tail() {
  if (!assert_equals_char("^ to tail is w", 'w', body_to_tail('^'))) {
    return false;
  }

  if (!assert_equals_char("v to tail is s", 's', body_to_tail('v'))) {
    return false;
  }
...
bool test_incr_x() {
  if (!assert_equals_int("incr_x '>' is 1", 1, incr_x('>'))) {
    return false;
  }

  if (!assert_equals_int("incr_x 'a' is -1", -1, incr_x('a'))) {
    return false;
  }

  if (!assert_equals_int("incr_x 'e' is 0", 0, incr_x('e'))) {
    return false;
  }
...
bool test_incr_y() {
  if (!assert_equals_int("incr_y 's' is 1", 1, incr_y('s'))) {
    return false;
  }

  if (!assert_equals_int("incr_y '^' is -1", -1, incr_y('^'))) {
    return false;
  }

  if (!assert_equals_int("incr_y 'e' is 0", 0, incr_y('e'))) {
    return false;
  }
